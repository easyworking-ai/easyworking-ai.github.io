#!/usr/bin/env python3
"""Render expanded EP02 radio episode with Edge TTS + post-processing."""
...
VOICES = {
    "ko": {
        "iro": "ko-KR-JiMinNeural",      # bright female
        "loop": "ko-KR-InJoonNeural",     # calm male
    },
    "en": {
        "iro": "en-US-AriaNeural",        # bright female
        "loop": "en-US-AndrewNeural",     # calm male
    },
    "ja": {
        "iro": "ja-JP-NanamiNeural",      # bright female
        "loop": "ja-JP-KeitaNeural",      # calm male
    },
}

# Scripts: list of (speaker, text) tuples
SCRIPTS = {
    "ko": [
...
def render_language(lang):
    """Render full episode for a language using edge-tts."""
    voices = VOICES[lang]
    script = SCRIPTS[lang]
    ...
    for i, (speaker, text) in enumerate(script):
        ...
        rate = "+5%" if speaker == "iro" else "-5%"
        pitch = "+2st" if speaker == "iro" else "-1st"
        ...
        cmd = [
            ...
            "--rate", rate,
            "--pitch", pitch,
            "--write-media", chunk_file,
        ]
        
        result = subprocess.run(cmd, capture_output=True, text=True, timeout=30)
        if result.returncode != 0:
            print(f"  ERROR chunk {i}: {result.stderr[:200]}")
        else:
...
def assemble_episode(lang, chunks):
    """Assemble chunks with music, transitions, and gaps into final MP3."""
    work = f"{SCRIPT_DIR}/{lang}_assemble"
    os.makedirs(work, exist_ok=True)
    
    # Step 1: Convert each MP3 chunk to WAV with consistent format
    wav_files = []
    ...
    # Step 2: Create silence files for gaps
    # Short pause between same speaker: 0.3s
    # Cross-speaker pause: 0.5s
    # Section transition: 1.5s
    subprocess.run([
        "ffmpeg", "-y", "-f", "lavfi", "-i",
        "anullsrc=channel_layout=mono:sample_rate=44100",
        "-t", "0.3", f"{work}/sil_short.wav"
    ], capture_output=True)
    subprocess.run([
        "ffmpeg", "-y", "-f", "lavfi", "-i",
        "anullsrc=channel_layout=mono:sample_rate=44100",
        "-t", "0.5", f"{work}/sil_mid.wav"
    ], capture_output=True)
    subprocess.run([
        "ffmpeg", "-y", "-f", "lavfi", "-i",
        "anullsrc=channel_layout=mono:sample_rate=44100",
        "-t", "1.5", f"{work}/sil_long.wav"
    ], capture_output=True)
    
    # Step 3: Build concat list with appropriate gaps
    # Section boundaries (insert long silence + transition)
    # Script structure for ko (37 lines):
    # 0-3: Intro (IRO) → section break before LOOP intro
    # 4-7: LOOP explains Opus 5 → section break
    # 8-13: IRO/LOOP discuss implications → section break
    # 14-20: Hugging Face attack → section break
    # 21-22: Transition to other news → section break
    # 23-28: Other news (Gemini, Cursor, Aurora) → section break
    # 29-33: Experiment + listener engagement → section break
    # 34-36: Outro
    
    # Define section breaks (insert long silence at these indices)
    section_breaks = {3, 7, 13, 20, 22, 28, 33}
    
    concat_list = []
    # Intro music
    concat_list.append(f"file '{SCRIPT_DIR}/intro_sting.wav'")
    ...
    for i, wav_file in enumerate(wav_files):
        ...
        if i in section_breaks:
            # Section transition: silence + transition sound + silence
            concat_list.append(f"file '{work}/sil_long.wav'")
            concat_list.append(f"file '{SCRIPT_DIR}/transition.wav'")
            concat_list.append(f"file '{work}/sil_long.wav'")
        elif i < len(wav_files) - 1:
            # Regular gap between speakers
            speaker_now = chunks[i].split("_")[-1].replace(".mp3", "")
            speaker_next = chunks[i+1].split("_")[-1].replace(".mp3", "")
    ...
    subprocess.run([
        ...
        "-filter_complex",
        f"[1:a]volume=0.15,afade=t=in:st=0:d=2,afade=t=out:st={{DUR}}:d=3[bg];"
...
if __name__ == "__main__":
    for lang in ["ko", "en", "ja"]:
        ...
        chunks = render_language(lang)
        print(f"  {len(chunks)} chunks rendered")
        
        print(f"  Assembling episode...")
        output = assemble_episode(lang, chunks)
