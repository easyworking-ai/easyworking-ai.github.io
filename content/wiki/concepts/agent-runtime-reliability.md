---
title: GPT-5 나와도 에이전트는 깨진다 — 실무에서 봉착한 5가지 신뢰성 문제
...
사내에서 에이전트를 쓰는 사람들은 이를 몸으로 느끼고 있었다. "이 모델이 더 똑똑하긴 한데, 응답이 느려서 실제 업무에는 못 쓰겠다"는 반응이 일상화되었다. 안정적 운영과 확장성을 고려하며 파운데이션 모델이 발전하고 있지만, 체감 품질은 여전히 지연과 실행 안정성에 묶여 있다.
