...
## 2.8조 매개변수는 어떻게 만들어졌나

2019년, GPT-2는 1억 2,400만 개의 매개변수를 가진 언어모델이었다. 2026년 7월, KimiK3는 2조 8,000억 개다. 7년간 모델 크기는 22,580배 커졌다.

하지만 이것이 단순한 스케일업이라면 이야기할 게 없다. 중요한 건 모델 내부 구조가 매 진화 단계마다 근본적으로 바뀌었다는 점이다. X(구 트위터)에서 ali(@waterloo\_intern)가 정리한 [From GPT2 to Kimi3, Explained](https://x.com/waterloo_intern/status/2081762065392541951) 아티클은 이 7년을 하나의 질문으로 관통한다:

> **고정된 공간에 정보를 어떻게 저장하고, 어떻게 갱신하고, 어떻게 꺼낼 것인가?**

각 아키텍처는 이 질문에 대해 다른 대답을 내놓는다. 이 글은 그 대답이 어떻게 바뀌어왔는지를 따라간다.
...
*원문 아티클을 바탕으로 학습 순서를 잡을 때 참고한 이미지입니다.*
